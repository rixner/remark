...
describe('Lexer', () => {
  describe('identifying tokens', () => {
    it('should recognize text', () => {
      lexer.lex('1').should.eql([{ type: 'text', text: '1' }]);
    });

    it('should treat empty source as empty text token', () => {
      lexer.lex('').should.eql([{ type: 'text', text: '' }]);
    });

    it('should recognize normal separator', () => {
      lexer.lex('\n---').should.eql([{ type: 'separator', text: '---' }]);
    });
    ...
    it('should recognize silent separator', () => {
      lexer
        .lex('\n<!-- break -->')
        .should.eql([{ type: 'separator', text: '<!-- break -->' }]);
    });

    it('should not preserve trailing line breaks of silent separators', () => {
      lexer
        .lex('\n<!-- break -->\n')
        .should.eql([{ type: 'separator', text: '<!-- break -->' }]);
    });

    it('should ignore additional whitespace inside of silent separator', () => {
      lexer
        .lex('\n<!--    break  -->')
        .should.eql([{ type: 'separator', text: '<!--    break  -->' }]);
    });
    ...
    it('should recognize notes separator', () => {
      lexer
        .lex('\n???\n')
        .should.eql([{ type: 'notes_separator', text: '???' }]);
    });

    it('should recognize code', () => {
      lexer.lex('    code').should.eql([{ type: 'code', text: '    code' }]);
    });

    it('should recognize inline code using single backticks', () => {
      lexer.lex('`code`').should.eql([{ type: 'text', text: '`code`' }]);
    });
    ...
    it('should recognize inline code containing escaped backticks', () => {
      lexer
        .lex('`` `code` ``')
        .should.eql([{ type: 'text', text: '`` `code` ``' }]);
    });

    it('should recognize fences', () => {
      lexer
        .lex('```\ncode```')
        .should.eql([{ type: 'fences', text: '```\ncode```' }]);
    });
    ...
    it('should ignore escaped content class', () => {
      lexer
        .lex('\\.class[content]')
        .should.eql([{ type: 'text', text: '.class[content]' }]);
    });

    it('should treat unclosed content class as text', () => {
      lexer
        .lex('text .class[content')
        .should.eql([{ type: 'text', text: 'text .class[content' }]);
    });
    ...
    it('should leave content class inside code as-is', () => {
      lexer
        .lex('    .class[x]')
        .should.eql([{ type: 'code', text: '    .class[x]' }]);
    });

    it('should leave content class inside inline code as-is', () => {
      lexer
        .lex('`.class[x]`')
        .should.eql([{ type: 'text', text: '`.class[x]`' }]);
    });
